...
def extract_long_str(line):
    line_list = [l for l in line.splitlines() if l]
    num = int(line_list.pop(0))
    sorted_line_list = sorted(line_list, key=len, reverse=True)
    long_str = ''
    for i in range(num):
        long_word = sorted_line_list[i] + '\n'
        long_str += long_word
...
# extract_long_str(test)
# extract_long_str(test2)
